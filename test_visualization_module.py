...
def demo_basic_plots():
    ...
    fig = plot_spend_vs_conversions(
        df, save_path=str(output_dir / "spend_vs_conversions.png"), show_plot=False
    )
    ...
    fig = plot_campaign_performance(
        df, save_path=str(output_dir / "performance_dashboard.png"), show_plot=False
    )
    ...
    fig = create_roi_trend_plot(
        df, save_path=str(output_dir / "roi_trend.png"), show_plot=False
    )
...
def demo_budget_allocation_plots():
    ...
    print(
        f"   ✓ Created budget data for {len(budget_data['campaign_names'])} campaigns"
    )
...
def create_synthetic_data():
    ...
    for i in range(n_records):
        ...
        conversions = int(
            clicks * np.random.uniform(0.01, 0.1)
        )  # 1-10% conversion rate

        # Revenue with some correlation to spend and conversions
        revenue = spend * np.random.uniform(0.8, 2.5) + conversions * np.random.uniform(
            10, 100
        )
