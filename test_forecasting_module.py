...
def test_baseline_forecasting():
    ...
    print(f"   Predictions range: {predictions['predicted_conversions'].min():.1f} to {predictions['predicted_conversions'].max():.1f}")
    
    # Test future forecasting
    print("\n2. Testing future forecasting...")
    future_forecasts = forecaster.forecast_future(df, future_periods=7, spend_scenario=200.0)
    print(f"   Generated {len(future_forecasts)} future forecasts")
    print(f"   Future predictions range: {future_forecasts['predicted_conversions'].min():.1f} to {future_forecasts['predicted_conversions'].max():.1f}")
...
def test_time_series_forecasting():
    ...
    quick_forecasts = forecast_conversions_time_series(df, future_periods=5, method='seasonal_naive')
    print(f"   Generated {len(quick_forecasts)} quick forecasts")
...
def test_model_selection():
    ...
    for i, row in comparison.head(3).iterrows():
        print(f"   {i+1}. {row['model']}: {row['mae_mean']:.2f} MAE (±{row['mae_std']:.2f})")
...
def test_end_to_end_forecasting():
    ...
    for i in range(min(5, len(forecasts))):
        ...
        if 'date' in row:
            print(f"   {row['date'].strftime('%Y-%m-%d')}: {row['predicted_conversions']:.1f} conversions "
                  f"(CI: {row['lower_bound']:.1f} - {row['upper_bound']:.1f})")
        else:
            print(f"   Period {i+1}: {row['predicted_conversions']:.1f} conversions "
                  f"(CI: {row['lower_bound']:.1f} - {row['upper_bound']:.1f})")
