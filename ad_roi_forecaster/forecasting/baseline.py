...
class BaselineForecaster:
    ...
    def predict(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        results['predicted_conversions'] = np.maximum(0, results['predicted_conversions'])
    ...
    def forecast_future(self, 
                       df: pd.DataFrame,
                       future_periods: int,
                       spend_scenario: Union[float, List[float], Dict[str, float]]) -> pd.DataFrame:
        """
    ...
    def _create_future_row(self, 
        ...
        if 'impressions' in avg_data and avg_data['spend'] > 0:
            row['impressions'] = int(spend * avg_data['impressions'] / avg_data['spend'])
        else:
            row['impressions'] = int(spend * 100)  # Default assumption
        
        # Assume clicks based on historical CTR
        if 'clicks' in avg_data and 'impressions' in avg_data and avg_data['impressions'] > 0:
...
def create_baseline_forecaster(df: pd.DataFrame, 
                             test_size: float = 0.2,
                             random_state: int = 42) -> Tuple[BaselineForecaster, Dict[str, float]]:
    """
...
def forecast_conversions_baseline(df: pd.DataFrame,
