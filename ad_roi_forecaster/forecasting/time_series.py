    ...
    def predict(self, periods: int) -> pd.DataFrame:
        ...
        for i, date in enumerate(future_dates):
            ...
            if len(conversions) >= self.seasonal_period:
                ...
                seasonal_offset = seasonal_index - (len(conversions) % self.seasonal_period)
...
class ARIMAForecaster:
    ...
    def fit(self, df: pd.DataFrame) -> 'ARIMAForecaster':
        ...
        # Create time series
        ts = pd.Series(
            daily_conversions['conversions'].values,
            index=daily_conversions['date']
        )
        ...
        try:
            ...
            self.is_trained = True
            logger.info(f"ARIMA model trained successfully. AIC: {self.fitted_model.aic:.2f}")
    ...
    def predict(self, periods: int) -> pd.DataFrame:
        ...
        conf_int = self.fitted_model.get_forecast(steps=periods).conf_int(alpha=1-self.confidence_level)
...
def decompose_time_series(df: pd.DataFrame, 
    ...
    if STATSMODELS_AVAILABLE and len(ts) >= 2 * period:
        ...
        return {
            ...
            'trend': pd.DataFrame({'date': decomposition.trend.index, 'values': decomposition.trend.values}),
            'seasonal': pd.DataFrame({'date': decomposition.seasonal.index, 'values': decomposition.seasonal.values}),
            'residual': pd.DataFrame({'date': decomposition.resid.index, 'values': decomposition.resid.values})
        }
    else:
        ...
        return {
            ...
            'residual': pd.DataFrame({'date': residual.index, 'values': residual.values})
        }


def forecast_conversions_time_series(df: pd.DataFrame,
...
def create_time_series_forecaster(df: pd.DataFrame,
                                 method: str = 'seasonal_naive',
                                 test_size: float = 0.2) -> Tuple[TimeSeriesForecaster, Dict[str, float]]:
    """
