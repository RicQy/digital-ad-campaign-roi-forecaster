...
class ModelSelector:
    ...
    def _evaluate_model(self, model: Any, train_df: pd.DataFrame, test_df: pd.DataFrame) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if isinstance(model, BaselineForecaster):
                ...
                daily_conversions = test_df_copy.groupby('date')['conversions'].sum().reset_index()
    ...
    def _time_series_cv_split(self, df: pd.DataFrame) -> List[Tuple[pd.DataFrame, pd.DataFrame]]:
        """
        ...
        min_train_size = max(2, n_samples // (self.cv_folds + 1))  # Reduced minimum train size
        
        splits = []
        for i in range(self.cv_folds):
            # Calculate split points
            train_end = min_train_size + (i + 1) * (n_samples - min_train_size) // self.cv_folds
            test_start = train_end
            test_end = min(test_start + max(1, (n_samples - min_train_size) // self.cv_folds), n_samples)
            
            if test_end > test_start and train_end >= 2:  # Ensure we have at least 2 samples for training
    ...
    def fit(self, df: pd.DataFrame) -> 'ModelSelector':
        ...
        for model_name, model_config in self.models.items():
            ...
            for fold, (train_df, test_df) in enumerate(cv_splits):
                try:
                    ...
                    logger.debug(f"Fold {fold + 1} - {model_name}: {scores[self.scoring]:.4f}")
            ...
            if fold_scores:
                ...
                for metric in ['mae', 'mse', 'rmse', 'mape', 'r2_score']:
                    scores = [score[metric] for score in fold_scores if not np.isinf(score[metric])]
                    if scores:
                        mean_scores[metric] = np.mean(scores)
                        std_scores[metric] = np.std(scores)
                    else:
                        mean_scores[metric] = float('inf') if metric != 'r2_score' else -float('inf')
                ...
                logger.info(f"Model {model_name} - Mean {self.scoring}: {mean_scores[self.scoring]:.4f} "
        ...
        if self.best_model_name:
            self.best_model = self._create_model(self.best_model_name, self.models[self.best_model_name])
    ...
    def get_best_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'model_type': self.models[self.best_model_name]['type'] if self.best_model_name else None
        }
...
def select_best_model(df: pd.DataFrame,
                     ...
                     cv_folds: int = 5,
                     scoring: str = 'mae',
                     confidence_level: float = 0.95) -> Tuple[Any, Dict[str, Any]]:
    """
...
def forecast_with_best_model(df: pd.DataFrame,
                           future_periods: int = 30,
                           spend_scenario: Optional[Union[float, List[float]]] = None,
                           models: Optional[Dict[str, Any]] = None,
                           cv_folds: int = 5,
                           scoring: str = 'mae',
                           confidence_level: float = 0.95) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
    ...
    if isinstance(selector.best_model, BaselineForecaster):
        ...
        forecasts = selector.best_model.forecast_future(df, future_periods, spend_scenario)
