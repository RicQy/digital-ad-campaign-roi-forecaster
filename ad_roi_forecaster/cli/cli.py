...
from ad_roi_forecaster.visualization.report import generate_pdf_report, generate_html_report
...
@click.option('--periods', '-p', default=30, type=int, 
              help='Number of periods to forecast (default: 30)')
@click.option('--spend-scenario', '-s', type=float, 
              help='Spend scenario for forecasting (default: use historical average)')
@click.option('--model', '-m', type=click.Choice(['auto', 'baseline', 'arima', 'seasonal_naive']), 
              default='auto', help='Model type to use for forecasting')
@click.option('--confidence', '-c', default=0.95, type=float, 
...
@click.option('--scoring', default='mae', type=click.Choice(['mae', 'mse', 'rmse', 'mape', 'r2_score']), 
              help='Scoring metric for model selection (default: mae)')
def forecast(input_path: str, output_path: str, periods: int, spend_scenario: Optional[float], 
             model: str, confidence: float, cv_folds: int, scoring: str):
...
@click.option('--budget', '-b', required=True, type=float, 
...
@click.option('--campaigns', '-c', multiple=True, 
...
def optimize(input_path: Optional[str], budget: float, start_date: Optional[datetime], 
             end_date: Optional[datetime], output_path: Optional[str], 
             campaigns: tuple, platforms: tuple):
    ...
    try:
        ...
        if input_path:
            ...
            if start_date and end_date:
                ...
                console.print(f"📅 Filtered to {len(df)} records in date range", style="yellow")
            
            # Filter by campaigns if provided
            if campaigns:
                df = df[df['campaign_id'].isin(campaigns)]
                console.print(f"🎪 Filtered to {len(df)} records for specified campaigns", style="yellow")
            
            # Filter by platforms if provided
            if platforms:
                df = df[df['platform'].isin(platforms)]
                console.print(f"🌐 Filtered to {len(df)} records for specified platforms", style="yellow")
            ...
            console.print("🔧 No input data provided, creating synthetic campaign data...")
        ...
        with Progress() as progress:
            ...
            # Create results
            results = _create_optimization_results(df, optimal_allocation, expected_roi, budget)
            progress.update(task, advance=100)
        ...
        if output_path:
            ...
            console.print(f"✅ Optimization results saved to {output_path}", style="green")
...
@click.option('--input', '-i', 'input_path', required=True, type=click.Path(exists=True), 
              help='Path to input CSV file containing campaign data')
@click.option('--output-dir', '-o', 'output_dir', required=True, type=click.Path(), 
              help='Directory to save generated reports')
@click.option('--format', '-f', 'report_format', multiple=True, 
              type=click.Choice(['html', 'pdf']), default=['html'], 
              help='Report format(s) to generate (default: html)')
@click.option('--title', '-t', default='Digital Ad Campaign ROI Analysis', 
              help='Report title')
@click.option('--company', '-c', default='Campaign Analytics', 
              help='Company name for report')
@click.option('--include-forecasts', is_flag=True, 
              help='Include forecasting analysis in report')
@click.option('--forecast-periods', default=30, type=int, 
              help='Number of periods to forecast for report (default: 30)')
@click.option('--include-optimization', is_flag=True, 
              help='Include budget optimization analysis in report')
@click.option('--optimization-budget', type=float, 
...
def report(input_path: str, output_dir: str, report_format: tuple, title: str, 
           company: str, include_forecasts: bool, forecast_periods: int, 
           include_optimization: bool, optimization_budget: Optional[float], dpi: int):
    ...
    try:
        ...
        with Progress() as progress:
            total_tasks = len(report_format) + (1 if include_forecasts else 0) + (1 if include_optimization else 0)
            ...
            if include_optimization:
                ...
                budget_allocation_data = {
                    ...
                    'current_allocation': df.groupby('campaign_id')['spend'].sum().tolist(),
                    'optimized_allocation': optimal_allocation.tolist(),
                    'roi_values': [(row['revenue'] - row['spend']) / row['spend'] * 100 
                                  for _, row in df.groupby('campaign_id').agg({'spend': 'sum', 'revenue': 'sum'}).iterrows()]
                }
                progress.update(main_task, advance=1)
            ...
            for fmt in report_format:
                ...
                if fmt == 'html':
                    output_file = output_path / f"campaign_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
                    ...
                    output_file = output_path / f"campaign_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
                    generate_pdf_report(
                        df=df,
                        output_path=str(output_file),
                        config=config,
                ...
                console.print(f"✅ {fmt.upper()} report saved to {output_file}", style="green")
                progress.update(main_task, advance=1)
        
        console.print(f"🎉 All reports generated successfully in {output_dir}", style="bold green")
...
def _display_data_summary(df: pd.DataFrame) -> None:
    ...
    table.add_row("Date Range", f"{df['date'].min()} to {df['date'].max()}" if 'date' in df.columns else "N/A")
    ...
    table.add_row("Unique Campaigns", f"{df['campaign_id'].nunique():,}" if 'campaign_id' in df.columns else "N/A")
    table.add_row("Platforms", f"{df['platform'].nunique():,}" if 'platform' in df.columns else "N/A")
    
    # Calculate overall ROI
    overall_roi = ((df['revenue'].sum() - df['spend'].sum()) / df['spend'].sum() * 100) if df['spend'].sum() > 0 else 0
...
def _display_forecast_summary(forecasts: pd.DataFrame) -> None:
    ...
    if 'predicted_conversions' in forecasts.columns:
        ...
        table.add_row("Total Predicted Conversions", f"{forecasts['predicted_conversions'].sum():.0f}")
        table.add_row("Average Daily Conversions", f"{forecasts['predicted_conversions'].mean():.1f}")
        table.add_row("Min Prediction", f"{forecasts['predicted_conversions'].min():.1f}")
        table.add_row("Max Prediction", f"{forecasts['predicted_conversions'].max():.1f}")
...
def _display_optimization_results(results: List[Dict], expected_roi: float) -> None:
    ...
    for result in results:
        ...
        change_pct = (change / result['current_spend'] * 100) if result['current_spend'] > 0 else 0
...
def _create_synthetic_campaign_data(budget: float, start_date: Optional[datetime], 
    ...
    campaigns = [
        {'campaign_id': 'campaign_001', 'platform': 'google', 'campaign_type': 'search'},
        {'campaign_id': 'campaign_002', 'platform': 'facebook', 'campaign_type': 'social'},
        {'campaign_id': 'campaign_003', 'platform': 'instagram', 'campaign_type': 'social'},
        {'campaign_id': 'campaign_004', 'platform': 'google', 'campaign_type': 'display'},
        {'campaign_id': 'campaign_005', 'platform': 'linkedin', 'campaign_type': 'social'},
    ]
...
def _create_optimization_results(df: pd.DataFrame, optimal_allocation: np.ndarray, 
                                expected_roi: float, budget: float) -> List[Dict]:
