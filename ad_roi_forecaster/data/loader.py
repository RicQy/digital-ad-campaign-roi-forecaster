...
class CampaignDataLoader:
    ...
    DEFAULT_COLUMN_MAPPING = {
        ...
        'target_audience': ['target_audience', 'Target_Audience', 'audience', 'segment'],
        'geographic_region': ['geographic_region', 'Geographic_Region', 'region', 'geo'],
    ...
    def _coerce_data_types(self, df: pd.DataFrame, column_mapping: Dict[str, str]) -> pd.DataFrame:
        """
        ...
        for standard_name, actual_name in column_mapping.items():
            ...
            try:
                ...
                self.logger.warning(f"Failed to convert {actual_name} to {target_dtype}: {e}")
        
        return df_copy
    
    def _standardize_column_names(self, df: pd.DataFrame, column_mapping: Dict[str, str]) -> pd.DataFrame:
        """
        ...
        """
        rename_mapping = {actual: standard for standard, actual in column_mapping.items()}
        ...
        for col in required_columns:
            if col not in df_renamed.columns:
                ...
                self.logger.warning(f"Missing required column {col}, filled with defaults")
    ...
    def _clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(df_clean) < initial_count:
            self.logger.warning(f"Removed {initial_count - len(df_clean)} rows with missing critical data")
        ...
        string_columns = ['campaign_name', 'campaign_type', 'platform', 
                         'target_audience', 'geographic_region']
    ...
    def load_multiple_csvs(self, 
        ...
        self.logger.info(f"Combined {len(file_paths)} CSV files into {len(combined_df)} rows")
