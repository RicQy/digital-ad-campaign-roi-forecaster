...
class CampaignRecord(BaseModel):
    ...
    geographic_region: Optional[str] = Field(None, description="Geographic targeting region")
    
    # Optional campaign settings
    budget_daily: Optional[float] = Field(None, ge=0, description="Daily budget limit")
    budget_total: Optional[float] = Field(None, ge=0, description="Total campaign budget")
    ...
    def validate_dataset_consistency(self):
        ...
        for record in self.records:
            ...
            if combination in seen_combinations:
                raise ValueError(f'Duplicate campaign_id and date combination: {combination}')
