...
class CampaignDataValidator:
    ...
    def _check_date_range(
        ...
        if start_date:
            ...
            if not out_of_bounds_start.empty:
                errors.append(
                    f"{len(out_of_bounds_start)} records with dates before {start_date}"
                )
        ...
        if end_date:
            ...
            if not out_of_bounds_end.empty:
                errors.append(
                    f"{len(out_of_bounds_end)} records with dates after {end_date}"
                )
    ...
    def _check_numeric_bounds(self) -> List[str]:
        ...
        if "spend" in self.data.columns and "revenue" in self.data.columns:
            ...
            if high_spend_ratio > 0:
                issues.append(
                    f"{high_spend_ratio} records with spend more than 10x revenue"
                )
    ...
    def _check_data_consistency(self) -> List[str]:
        ...
        if all(col in self.data.columns for col in ["campaign_id", "date"]):
            ...
            if duplicates > 0:
                warnings.append(
                    f"{duplicates} duplicate campaign_id + date combinations"
                )
        ...
        if "campaign_type" in self.data.columns:
            ...
            if invalid_count > 0:
                warnings.append(
                    f"{invalid_count} records with invalid campaign type values"
                )
    ...
    def _check_statistical_outliers(self) -> List[str]:
        ...
        for col in numeric_columns:
            if col in self.data.columns:
                ...
                if outliers > 0:
                    warnings.append(
                        f"{outliers} statistical outliers detected in {col}"
                    )
    ...
    def validate(
        ...
        if self.data.empty:
            ...
            return ValidationResult(
                is_valid=False, errors=errors, warnings=warnings, record_count=0
            )
        ...
        for issue in numeric_issues:
            if any(
                ...
                for keyword in ["negative", "exceed", "invalid"]
            ):
                errors.append(issue)
            else:
