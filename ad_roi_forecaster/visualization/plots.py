...
def plot_budget_allocation(
    ...
    bars1 = ax1.bar(x - width/2, current_allocation, width, label='Current Allocation', alpha=0.8)
    bars2 = ax1.bar(x + width/2, optimized_allocation, width, label='Optimized Allocation', alpha=0.8)
    ...
    fig.suptitle(f'{title}\nTotal Budget: ${total_current:,.0f} → ${total_optimized:,.0f} | Avg ROI: {avg_roi:.1f}%', fontsize=14)
...
def plot_campaign_performance(
    ...
    if 'campaign_type' in df.columns:
        type_conversion = df.groupby('campaign_type')['conversion_rate'].mean().sort_values(ascending=True)
        ax3.barh(type_conversion.index, type_conversion.values, color='lightblue', alpha=0.8)
    ...
    if 'date' in df.columns:
        ...
        ax4.plot(daily_performance['date'], daily_performance['spend'], 
                label='Spend', color='red', linewidth=2)
        ax4.plot(daily_performance['date'], daily_performance['revenue'], 
                label='Revenue', color='green', linewidth=2)
        
        # Plot conversions on secondary axis
        ax4_twin.bar(daily_performance['date'], daily_performance['conversions'], 
                    alpha=0.3, color='blue', label='Conversions')
...
def create_roi_trend_plot(
    ...
    # Calculate daily ROI
    daily_roi = df.groupby('date').apply(
    ...
    p = np.poly1d(z)
    ax.plot(daily_roi['date'], p(x_numeric), "r--", alpha=0.8, linewidth=2, label=f'Trend (slope: {z[0]:.4f})')
