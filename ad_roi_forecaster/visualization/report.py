...
class ReportGenerator:
    ...
    def generate_html_report(
        ...
        plot_images = self._generate_plots_for_html(
            df, forecast_data, budget_allocation_data
        )
    ...
    def _calculate_summary_stats(self, df: pd.DataFrame) -> None:
        ...
        self.summary_stats["overall_roi"] = (
            ...
            / self.summary_stats["total_spend"]
            * 100
        )
        ...
        self.summary_stats["overall_ctr"] = (
            self.summary_stats["total_clicks"]
            / self.summary_stats["total_impressions"]
            * 100
        )
        self.summary_stats["overall_conversion_rate"] = (
            self.summary_stats["total_conversions"]
            / self.summary_stats["total_clicks"]
            * 100
        )
        ...
        if "platform" in df.columns:
            ...
            platform_stats["roi"] = (
                (platform_stats["revenue"] - platform_stats["spend"])
                / platform_stats["spend"]
                * 100
            )
        ...
        if "campaign_type" in df.columns:
            ...
            type_stats["roi"] = (
                (type_stats["revenue"] - type_stats["spend"])
                / type_stats["spend"]
                * 100
            )
            self.summary_stats["campaign_type_performance"] = type_stats.to_dict(
                "index"
            )
    ...
    def _create_cover_page(self, pdf_pages: pdf_backend.PdfPages) -> None:
        ...
        # Company name
        ax.text(
            0.5, 0.7, self.config.company_name, fontsize=16, ha="center", va="center"
        )
    ...
    def _create_executive_summary_page(
        self,
        pdf_pages: pdf_backend.PdfPages,
        df: pd.DataFrame,
        additional_info: Optional[Dict] = None,
    ) -> None:
        ...
        ax.text(0.1, 0.75, key_findings, fontsize=11, ha="left", va="top", wrap=True)
        ...
        ax.text(0.1, 0.45, metrics_text, fontsize=11, ha="left", va="top")
        ...
        if additional_info:
            ...
            ax.text(0.1, 0.15, info_text, fontsize=11, ha="left", va="top")

        pdf_pages.savefig(fig, bbox_inches="tight")
        plt.close(fig)

    def _create_data_overview_page(
        self, pdf_pages: pdf_backend.PdfPages, df: pd.DataFrame
    ) -> None:
        ...
        ax.text(0.1, 0.85, dataset_info, fontsize=11, ha="left", va="top")
        ...
        if "platform" in df.columns and "platform_performance" in self.summary_stats:
            ...
            ax.text(0.1, 0.6, platform_text, fontsize=11, ha="left", va="top")

        # Campaign type breakdown
        if (
            "campaign_type" in df.columns
            and "campaign_type_performance" in self.summary_stats
        ):
            type_text = "Campaign Type Performance:\n\n"
            for camp_type, stats in self.summary_stats[
                "campaign_type_performance"
            ].items():
                type_text += f"• {camp_type.title()}: ${stats['spend']:,.0f} spend, ${stats['revenue']:,.0f} revenue, {stats['roi']:.1f}% ROI\n"

            ax.text(0.1, 0.3, type_text, fontsize=11, ha="left", va="top")
    ...
    def _add_optimization_plots(
        self, pdf_pages: pdf_backend.PdfPages, budget_allocation_data: Dict
    ) -> None:
    ...
    def _create_detailed_analysis_page(
        self, pdf_pages: pdf_backend.PdfPages, df: pd.DataFrame
    ) -> None:
        ...
        ax.text(0.1, 0.85, stats_text, fontsize=11, ha="left", va="top")
    ...
    def _create_recommendations_page(
        ...
        ax.text(0.1, 0.85, recommendations, fontsize=11, ha="left", va="top")

        pdf_pages.savefig(fig, bbox_inches="tight")
        plt.close(fig)

    def _generate_plots_for_html(
        self,
        df: pd.DataFrame,
        forecast_data: Optional[pd.DataFrame],
        budget_allocation_data: Optional[Dict],
    ) -> Dict[str, str]:
    ...
    def _fig_to_base64(self, fig: plt.Figure) -> str:
        ...
        fig.savefig(
            img_buffer, format="png", dpi=self.config.plot_dpi, bbox_inches="tight"
        )
        img_buffer.seek(0)
    ...
    def _create_html_content(
        ...
        for rec in recommendations_list:
            if rec.strip():
                recommendations_html += (
                    f'<div class="recommendation">{rec.strip()}</div>'
                )
    ...
    def _generate_key_findings(self, df: pd.DataFrame) -> str:
        ...
        if self.summary_stats["overall_roi"] > 0:
            findings.append(
                f"Campaigns generated a positive ROI of {self.summary_stats['overall_roi']:.1f}%"
            )
        else:
            findings.append(
                f"Campaigns resulted in a negative ROI of {self.summary_stats['overall_roi']:.1f}%"
            )

        # Best performing platform
        if "platform_performance" in self.summary_stats:
            best_platform = max(
                self.summary_stats["platform_performance"].items(),
                key=lambda x: x[1]["roi"],
            )
            findings.append(
                f"Best performing platform: {best_platform[0].title()} with {best_platform[1]['roi']:.1f}% ROI"
            )

        # Conversion insights
        if self.summary_stats["overall_conversion_rate"] > 2:
            findings.append(
                "Strong conversion performance with above-average conversion rates"
            )
        elif self.summary_stats["overall_conversion_rate"] < 1:
            findings.append(
                "Conversion rates below industry average, indicating optimization opportunities"
            )
    ...
    def _generate_statistical_analysis(self, df: pd.DataFrame) -> str:
        ...
        df_copy = df.copy()
        df_copy["roi"] = (
            (df_copy["revenue"] - df_copy["spend"]) / df_copy["spend"] * 100
        )
    ...
    def _generate_recommendations(self, df: pd.DataFrame) -> str:
        ...
        if self.summary_stats["overall_roi"] < 0:
            recommendations.append(
                "• Focus on improving ROI by reducing spend on underperforming campaigns"
            )
            recommendations.append(
                "• Analyze high-performing campaigns and replicate their strategies"
            )
            ...
            recommendations.append(
                "• Explore scaling successful campaigns to new markets or audiences"
            )
        ...
        if "platform_performance" in self.summary_stats:
            platform_rois = {
                k: v["roi"]
                for k, v in self.summary_stats["platform_performance"].items()
            }
            best_platform = max(platform_rois, key=platform_rois.get)
            worst_platform = min(platform_rois, key=platform_rois.get)

            recommendations.append(
                f"• Increase investment in {best_platform.title()} campaigns (highest ROI)"
            )
            if platform_rois[worst_platform] < 0:
                recommendations.append(
                    f"• Consider reducing or optimizing {worst_platform.title()} campaigns (negative ROI)"
                )
        ...
        recommendations.append(
            "• Implement automated bid adjustments based on performance data"
        )
        recommendations.append(
            "• Set up regular performance monitoring and optimization reviews"
        )
...
def generate_pdf_report(
    ...
    return generator.generate_pdf_report(
        df, output_path, forecast_data, budget_allocation_data, additional_info
    )
