...
def test_optimize_roi():
    ...
    assert len(allocation) == len(
        dataset.records
    ), "Allocation should have same length as records"
...
def test_optimize_roi_with_zero_budget():
    ...
    assert all(
        alloc == 0 for alloc in allocation
    ), "All allocations should be zero with zero budget"
